...
function render_admin_head(string $active): void
{
    ...
    $pageTitle = ($GLOBALS['pageTitle'] ?? 'Admin') . ' — obitickets';
...
<div class="admin-shell">
  ...
  <aside class="admin-sidebar" id="adminSidebar">
    <a class="admin-logo" href="/admin.php">
      ...
      <span>obitickets <em>admin</em></span>
    </a>
    <nav class="admin-nav">
      <?php foreach ($sections as $section): ?>
        ...
        ?>
        <?php if ($section['label']): ?><div class="admin-nav-label"><?= htmlspecialchars($section['label']) ?></div><?php endif; ?>
        <?php foreach ($visibleItems as $item): ?>
          <a class="admin-nav-item<?= $active === $item['key'] ? ' active' : '' ?>" href="<?= htmlspecialchars($item['href']) ?>">
    ...
    <a class="admin-nav-item admin-nav-exit" href="/">
  ...
  <div class="admin-main">
    <header class="admin-topbar">
      ...
      <div class="admin-topbar-profile">
        ...
        <a class="btn btn-line" style="padding:8px 16px; font-size:0.82rem" href="/logout.php">Log out</a>
      </div>
    </header>
